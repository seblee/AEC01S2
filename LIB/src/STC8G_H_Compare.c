...
void CMP_Inilize(CMP_InitDefine *CMPx)
{
    ...
    if (CMPx->CMP_P_Select == CMP_P_ADC)
        CMPCR1 |= PIS;  //比较器输入正极选择, CMP_P_P37: 选择P3.7做正输入, CMP_P_ADC: 由ADC模拟输入端做正输入.
    if (CMPx->CMP_N_Select == CMP_N_P36)
        CMPCR1 |= NIS;  //比较器输入负极选择, CMP_N_GAP: 选择内部BandGap经过OP后的电压做负输入, CMP_N_P36: 选择P3.6做负输入.
    if (CMPx->CMP_Outpt_En == ENABLE)
        CMPCR1 |= CMPOE;  //允许比较结果输出到P3.4/P4.1,   ENABLE,DISABLE
